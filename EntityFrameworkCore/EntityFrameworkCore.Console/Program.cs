...
//Dopo progetto Entity.API metto qua il collegamento con il DB per comodità
SpecialFolder folder = Environment.SpecialFolder.LocalApplicationData;
string path = Environment.GetFolderPath(folder);
string dbPath = Path.Combine(path, "FootballLeagueDbContext.db");
...
using FootballLeagueDbContext context = new FootballLeagueDbContext(optionsBuilder.Options);

#region CALL COMMENTATE SQLite
//Simple Insert with override SaveChanges()
...
#region SQL SERVER
//SQL Server
using var sqlServerContext = new FootballLeageSqlServerDbContext();

//Get history name of a record
await GetHistoryRecord();
async Task GetHistoryRecord()
{
    var teamHistory = await sqlServerContext.Teams.TemporalAll()
                                                  .Where(q => q.Id == 1)
                                                  .Select(team => new
                                                  {
                                                      Name = team.Name,
...
{
    int idToRemove = 1;
    League deletedLeague = await context.Leagues.FindAsync(idToRemove);
    deletedLeague.isDeleted = true;
    await context.SaveChangesAsync();

    //Posso farlo qua il filtraggio, o in modo globale in LeagueConfiguration
    //List<League> leagues = await context.Leagues.Where(q => !q.isDeleted).ToListAsync();

    //Global Query Filter attivo
    List<League> leagues = await context.Leagues.ToListAsync();
    foreach (League league in leagues)
    {
        Console.WriteLine($"{league.Name}");
    ...
    //Ignoro il Global Query Filter
    leagues = await context.Leagues.IgnoreQueryFilters().ToListAsync();
    foreach (League league in leagues)
    {
        Console.WriteLine($"{league.Name}");
    }
...
{
    int idToFind = 1;
    Team team = await context.Teams.FindAsync(idToFind);
    team.Name = "New Team With Concurrency Check";
    //team.Version = Guid.NewGuid(); lo sposto in SaveChanges() in DbContext
...
//Transactions
await TransactionSupport();
async Task TransactionSupport()
{
    League newLeague = new League
    {
    ...
    await context.AddRangeAsync(newTeams);
    await context.SaveChangesAsync();

    //Se transactionLeague, transactionCoach va a buon fine, ma transactionTeams falisce, abbiamo un Incomplete Operations
    //Se uno falisce, nessuno delle 3 deve essere aggiunto, devo usare Transaction

    var transaction = context.Database.BeginTransaction();

    League transactionLeague = new League
    {
        Name = "Testing Transactions"
    };
    await context.AddAsync(transactionLeague);
    await context.SaveChangesAsync();

    transaction.CreateSavepoint("CreatedLeague"); //checkpoint

    Coach transactionCoach = new Coach
    {
        Name = "New Team 1",
    };
    await context.AddAsync(transactionLeague);
    ...
    transaction.CreateSavepoint("CreatedCoach"); //checkpoint

    List<Team> transactionTeams = new List<Team>
    {
        new Team
        {
            Name = "New Team 1",
            LeagueId = transactionLeague.Id,
            CoachId = transactionCoach.Id
        },
        new Team
        {
            Name = "New Team 2",
    ...
    await context.AddRangeAsync(transactionTeams);
    await context.SaveChangesAsync();

    try
    {
        await transaction.CommitAsync(); //Se tutte e 3 a buon fine, commita
    }
    catch (Exception ex)
    {
        transaction.Rollback(); //Altimenti torna indietro
        transaction.RollbackToSavepoint("CreatedLeague"); //Posso anche tornare al 'checkpoint' che voglio
        transaction.RollbackToSavepoint("CreatedCoach");
        throw;
    }
}
